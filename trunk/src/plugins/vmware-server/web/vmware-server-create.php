...
function redirect_mgmt($strMsg, $currenttab = 'tab0', $vmware_server_id) {
    $url = 'vmware-server-manager.php?strMsg='.urlencode($strMsg).'&currenttab='.$currenttab.'&action=refresh&identifier[]='.$vmware_server_id;
			...
			if (strlen($vmware_server_id)) {
                ...
                if (strlen($vmware_server_disk)) {
                    if (!validate_input($vmware_server_disk, 'number')) {
                        $strMsg .= "Invalid vm disk size. Not creating the vm on VMware Server 1 Host $vmware_server_id<br>";
                        redirect($strMsg, "tab0", $vmware_server_id);
                    }
                }
