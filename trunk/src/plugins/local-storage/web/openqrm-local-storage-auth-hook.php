		...
		switch($cmd) {
			case "start":
				...
				if ($run_disable_deployment_export == 1) {
					...
					$auth_deployment_stop_cmd = "(ln -sf $OPENQRM_SERVER_BASE_DIR/openqrm/plugins/local-storage/web/openqrm-local-storage-auth-hook.php $OPENQRM_SERVER_BASE_DIR/openqrm/web/boot-service/openqrm-local-storage-export-auth-hook.$appliance_id.php && screen -dm wget -q -O /dev/null \"http://localhost/openqrm/boot-service/openqrm-local-storage-export-auth-hook.$appliance_id.php?bgcmd=stop_deployment_auth&appliance_id=$appliance_id\" && rm -f $OPENQRM_SERVER_BASE_DIR/openqrm/web/boot-service/openqrm-local-storage-export-auth-hook.$appliance_id.php $stop_deployment_hook_file)";
	...
	*/
	...
	function storage_auth_deployment_stop_in_background($appliance_id) {
		...
		if (strlen($transfer_from_nfs_param)) {
			...
			$event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "openqrm-local-storage-auth-hook.php", "Install-from-NFS: Authenticating $resource_ip on storage id $tp_storage_id:$tp_storage_ip:$tp_image_rootdevice", "", "", 0, 0, $appliance_id);
