...
<meta http-equiv="refresh" content="0; URL=vmware-server2-manager.php?currenttab=tab0&vmware_server_id=<?php echo $vmware_server_id; ?>&strMsg=Processing <?php echo $vmware_server_command; ?>">
</head>
<body>
...
$VMwareDir = $_SERVER["DOCUMENT_ROOT"].'/openqrm/base/plugins/vmware-server2/vmware-server2-stat';

$event = new event();

// user/role authentication
if ($OPENQRM_USER->role != "administrator") {
	$event->log("authorization", $_SERVER['REQUEST_TIME'], 1, "vmware-server2-action", "Un-Authorized access to vmware-server2-actions from $OPENQRM_USER->name", "", "", 0, 0, 0);
	exit();
}
...
unset($vmware_server_fields["vmware_server_command"]);
	...
	$event->log("$vmware_server_command", $_SERVER['REQUEST_TIME'], 5, "vmware-server2-action", "Processing command $vmware_server_command", "", "", 0, 0, 0);
	switch ($vmware_server_command) {
		...
		case 'start':
			...
			$vmware_server = new resource();
			$vmware_server->get_instance_by_id($vmware_appliance->resources);
			$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/vmware-server2/bin/openqrm-vmware-server2 start -n $vmware_server_name -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
			$vmware_server->send_command($vmware_server->ip, $resource_command);
			break;
		...
		case 'stop':
			...
			$vmware_server = new resource();
			$vmware_server->get_instance_by_id($vmware_appliance->resources);
			$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/vmware-server2/bin/openqrm-vmware-server2 stop -n $vmware_server_name -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
			$vmware_server->send_command($vmware_server->ip, $resource_command);
			break;
		...
		case 'reboot':
			...
			$vmware_server = new resource();
			$vmware_server->get_instance_by_id($vmware_appliance->resources);
			$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/vmware-server2/bin/openqrm-vmware-server2 reboot -n $vmware_server_name -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
			$vmware_server->send_command($vmware_server->ip, $resource_command);
			break;
		...
		case 'remove':
			...
			$vmware_server = new resource();
			$vmware_server->get_instance_by_id($vmware_appliance->resources);
			$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/vmware-server2/bin/openqrm-vmware-server2 remove -n $vmware_server_name -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
			$vmware_server->send_command($vmware_server->ip, $resource_command);
			break;
		...
		case 'add':
			...
			$vmware_server = new resource();
			$vmware_server->get_instance_by_id($vmware_appliance->resources);
			$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/vmware-server2/bin/openqrm-vmware-server2 add -n $vmware_server_name -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
			$vmware_server->send_command($vmware_server->ip, $resource_command);
			break;
		...
		case 'delete':
			...
			$vmware_server = new resource();
			$vmware_server->get_instance_by_id($vmware_appliance->resources);
			$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/vmware-server2/bin/openqrm-vmware-server2 delete -n $vmware_server_name -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
			$vmware_server->send_command($vmware_server->ip, $resource_command);
			break;
