		...
		switch($cmd) {
			case "start":
				...
				if ($run_disable_deployment_export == 1) {
					$stop_deployment_hook_file = "/tmp/openqrm-nfs-deployment-export-auth-hook.$appliance_id";
					$auth_deployment_stop_cmd = "(ln -sf $OPENQRM_SERVER_BASE_DIR/openqrm/plugins/nfs-storage/web/openqrm-nfs-deployment-auth-hook.php $OPENQRM_SERVER_BASE_DIR/openqrm/web/boot-service/openqrm-nfs-deployment-export-auth-hook.$appliance_id.php && screen -dm wget -q -O /dev/null \"http://localhost/openqrm/boot-service/openqrm-nfs-deployment-export-auth-hook.$appliance_id.php?bgcmd=stop_deployment_auth&appliance_id=$appliance_id\" && rm -f $OPENQRM_SERVER_BASE_DIR/openqrm/web/boot-service/openqrm-nfs-deployment-export-auth-hook.$appliance_id.php $stop_deployment_hook_file)";
	...
	function storage_auth_stop_in_background($appliance_id) {
		...
		while(1) {
			...
			if (!strcmp($resource->state, "active")) {
				$event->log("storage_auth_stop_in_background", $_SERVER['REQUEST_TIME'], 5, "openqrm-nfs-deployment-auth-hook.php", "Resource $resource_ip is idle again, applying stop auth for image $image_name", "", "", 0, 0, $appliance_id);
				break;				
			}
			if ($loop > 500) {
				$event->log("storage_auth_stop_in_background", $_SERVER['REQUEST_TIME'], 2, "openqrm-nfs-deployment-auth-hook.php", "Timeout for stop auth hook, exiting !", "", "", 0, 0, $appliance_id);
				return;
			}
			sleep(2);
			$loop++;
	...
	*/
	...
	function storage_auth_deployment_stop_in_background($appliance_id) {
		...
		while(1) {
			$resource->get_instance_by_id($appliance->resources);
			if (!strcmp($resource->state, "active")) {
				$event->log("storage_auth_deployment_stop_in_background", $_SERVER['REQUEST_TIME'], 5, "openqrm-nfs-deployment-auth-hook.php", "Resource $resource_ip is active now, applying stop auth for deployment exports", "", "", 0, 0, $appliance_id);
				break;				
			}
			if ($loop > 500) {
				$event->log("storage_auth_deployment_stop_in_background", $_SERVER['REQUEST_TIME'], 2, "openqrm-nfs-deployment-auth-hook.php", "Timeout for deployment stop auth hook image $image_name, exiting !", "", "", 0, 0, $appliance_id);
				return;
			}
			sleep(2);
			$loop++;
