...
unset($iscsi_storage_fields["iscsi_storage_command"]);
	...
	switch ($iscsi_storage_command) {
		...
		case 'add_lun':
			...
			$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/iscsi-storage/bin/openqrm-iscsi-storage add -n $iscsi_storage_image_name -m $iscsi_storage_image_size -i $image_password -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
			$storage_resource->send_command($storage_resource->ip, $resource_command);
			sleep($refresh_delay);
			break;
		...
		case 'remove_lun':
			...
			$storage_resource = new resource();
			$storage_resource->get_instance_by_id($storage->resource_id);
			$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/iscsi-storage/bin/openqrm-iscsi-storage remove -n $iscsi_storage_image_name -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
			$storage_resource->send_command($storage_resource->ip, $resource_command);
			sleep($refresh_delay);
			break;
		...
		case 'snap_lun':
			...
			$storage_resource = new resource();
			$storage_resource->get_instance_by_id($storage->resource_id);
			$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/iscsi-storage/bin/openqrm-iscsi-storage snap -n $iscsi_storage_image_name -s $iscsi_storage_image_snapshot_name -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
			$storage_resource->send_command($storage_resource->ip, $resource_command);
			sleep($refresh_delay);
			break;
		default:
