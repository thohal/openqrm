...
function storage_auth_function($cmd, $appliance_id) {
	...
	switch($cmd) {
		case "start":
			$event->log("openqrm_new_appliance", $_SERVER['REQUEST_TIME'], 5, "openqrm-dns-appliance-hook.php", "Authenticating $image_name to resource $resource_mac", "", "", 0, 0, $appliance_id);
			$auth_start_cmd = "$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/$deployment_plugin_name/bin/openqrm-$deployment_plugin_name auth -n $image_name -i $resource_mac";
...
function storage_auth_stop_in_background($appliance_id) {
	...
	$auth_stop_cmd = "$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/$deployment_plugin_name/bin/openqrm-$deployment_plugin_name auth -n $image_name -i 00:00:00:00:00:00";
