...
if(htmlobject_request('action') != '') {
	switch (htmlobject_request('action')) {
        ...
        case 'initialyze':
            ...
            // restore last backup ?
            if ($oqc_db_restore == 1) {
                $config_command = "$OPENQRM_SERVER_BASE_DIR/openqrm/sbin/openqrm-exec -i 127.0.0.1 -t $cmd_token -c \"/etc/init.d/openqrm-server init_config restore\"";
            } else {
                $config_command = "$OPENQRM_SERVER_BASE_DIR/openqrm/sbin/openqrm-exec -i 127.0.0.1 -t $cmd_token -c \"/etc/init.d/openqrm-server init_config\"";
            }
            shell_exec($config_command);
